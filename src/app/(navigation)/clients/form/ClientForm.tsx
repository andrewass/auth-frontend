...
export default function ClientForm({ client }: Props) {
	...
	const [step1FormData, setStep1FormData] = useState<ClientStep1FormData>(
		new ClientStep1FormData(client),
	);
	const [step2FormData, setStep2FormData] = useState<ClientStep2FormData>(
		new ClientStep2FormData(client),
	);
	...
	const steps = [
		<ClientFormStep1
			...
			updateFormData={setStep1FormData}
		/>,
		<ClientFormStep2
			key={1}
			formData={step2FormData}
			updateFormData={setStep2FormData}
		/>,
	...
	return (
		<div className="flex flex-col gap-14 w-2/3">
			...
			<div className="w-2/3 h-2/5 mx-auto">{steps[currentStep]}</div>
